...
export const getCurrentCustomer = query({
  handler: async (ctx) => {
    ...
    // Look for existing customer linked to this user (org optional)
    let customer = await ctx.db
      .query("customers")
      .withIndex("by_user", (q) => q.eq("userId", userId))
      .first();

    // If no customer record exists yet, auto-create a floating one
    if (!customer) {
      const now = Date.now();
      const customerId = await ctx.db.insert("customers", {
        userId,
        firstName: "New",           // fallback – in real app, pull from user profile or prompt user to edit
        lastName: "Customer",
        email: undefined,
        phone: undefined,
        createdAt: now,
        updatedAt: now,
        // organizationId intentionally omitted (optional in schema)
      });

      customer = await ctx.db.get(customerId)!;

      // Optional: create a basic audit log entry (no org)
      await ctx.db.insert("auditLogs", {
        ...
        metadata: { autoCreated: true, floating: true },
        createdAt: now,
      });
    }
